defmodule Schedules.HoursOfOperation do
  ...
  * sunday
  ...
  def hours_of_operation(route_id_or_ids, date \\ Util.service_date()) do
    ...
    |> Enum.chunk_every(6) # 3 dates * 2 directions == 6 responses per route
  ...
  * sunday, direction 1
  ...
  def week_dates(today) do
    ...
    [
      Date.add(today, 8 - dow), # Monday
      Date.add(today, Integer.mod(6 - dow, 7)), # Saturday, not going back in time
      Date.add(today, 7 - dow) # Sunday
    ]
  end
  ...
  def parse_responses([
    {:ok, week_response_0}, {:ok, saturday_response_0}, {:ok, sunday_response_0},
    {:ok, week_response_1}, {:ok, saturday_response_1}, {:ok, sunday_response_1}
  ]) do
  ...
  defp merge_departure(%Departures{} = first, %Departures{} = second) do
    %Departures{
      first_departure: Enum.min_by([first.first_departure, second.first_departure],
        &DateTime.to_unix(&1, :nanoseconds)),
      last_departure: Enum.max_by([first.last_departure, second.last_departure],
        &DateTime.to_unix(&1, :nanoseconds))
    }
  end
