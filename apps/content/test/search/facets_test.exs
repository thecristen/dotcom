defmodule Content.Search.FacetsTest do
  ...
  describe "build/3" do
    test "search results are filtered out" do
      facets = "type" |> build([{"search_result", 5}, {"page", 5}, {"event", 3}], ["page"]) |> Map.get("type")
  ...
  describe "facet_responses" do
    ...
    test "returns response and facet response when content types are given", %{search_fn: search_fn} do
