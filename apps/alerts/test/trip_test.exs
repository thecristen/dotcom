defmodule Alerts.TripTest do
  ...
  test "can query multiple trip IDs" do
    ...
    alert_both = Alert.new(informed_entity: [
          %IE{trip: @trip_id},
          %IE{trip: "other_trip"}
        ])
  ...
  test "includes delays that apply to the route" do
    ...
    suspension = Alert.new(
      effect: :suspension,
      informed_entity: [%IE{route: @route_id}]
    )
    wrong_route = Alert.new(
      header: "Wrong Route",
      effect: :delay,
      informed_entity: [%IE{route: "other route"}])
    wrong_effect = Alert.new(
      header: "Wrong Effect",
      informed_entity: [%IE{route: @route_id}])
  ...
  test "includes delays that are active at :time" do
    ...
    wrong_alert = Alert.new(informed_entity: [%IE{trip: @trip_id}],
                        active_period: [])
