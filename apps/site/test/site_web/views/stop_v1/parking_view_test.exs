defmodule SiteWeb.StopV1View.ParkingViewTest do
  @moduledoc false
  ...
  describe "utilization/1" do
    ...
    test "returns general message if parking spots fill up quickly but no time available" do
      message = utilization(%{@lot | utilization: %ParkingLot.Utilization{typical: 24}})
      assert safe_to_string(message) =~ "fill up quickly."
    end
  end
  ...
  describe "capacity/1" do
    ...
    test "prints No parking if capacity data unavailable" do
      out = safe_to_string(capacity(nil))
      assert out =~ "No MBTA parking."
    end
  end
  ...
  describe "payment/1" do
    ...
    test "prints out no payment info if it's not available" do
      out = safe_to_string(payment(nil))
      assert out =~ "No payment information available"
    end
  end
  ...
  describe "manager/1" do
    ...
    test "prints out not available if no management info" do
      out = safe_to_string(manager(nil))
      assert out =~ "No contact information available"
    end
  end
