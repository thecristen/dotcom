defmodule SiteWeb.FareView.DescriptionTest do
  ...
  describe "description/2" do
    ...
    test "fare description for month is describes the modes it can be used on" do
      result = %Fare{name: {:zone, "5"}, duration: :month, mode: :commuter_rail}
      |> description(%{})
      |> List.flatten
      ...
      assert body =~ "Travel beyond the designated zone on your pass will cost an additional interzone fare."
    end
    ...
    test "fare description for month mticket describes where it can be used" do
      ...
      assert [
        ...
        == fare |> description(%{}) |> Enum.map(fn(html) -> Phoenix.HTML.safe_to_string(html) end)
    end
    ...
    test "mentions zone 1a fares as part of the description for month passes on subway (regular fare)" do
      ...
      assert body =~ "CharlieTicket also valid for travel on Commuter Rail Zone 1A and Charlestown Ferry."
    end
  ...
  describe "transfers/1" do
    ...
    test "if fare is somewhere in the middle, lists fare differences along with transfers" do
      inner_express_fare =
        List.first(Fares.Repo.all(name: :inner_express_bus, duration: :single_trip, media: [:charlie_card]))

      fare = %Fare{mode: :bus, name: :local_bus, duration: :single_trip, media: [:charlie_card],
