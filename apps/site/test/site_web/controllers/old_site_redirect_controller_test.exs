defmodule SiteWeb.OldSiteRedirectControllerTest do
  ...
  describe "/schedules_and_maps" do
    test "redirects to mode root", %{conn: conn} do
      assert redirected_to(get(conn, "/schedules_and_maps"), :moved_permanently) == mode_path(conn, :index)
    end

    test "Redirects to new schedules based on old route", %{conn: conn} do
      old_url = "/schedules_and_maps/anything?route=RED"
      assert redirected_to(get(conn, old_url), :moved_permanently) =~ schedule_path(SiteWeb.Endpoint, :show, "Red")
    end
    ...
    test "Subway stop redirected to subway stops page", %{conn: conn} do
      ...
      assert redirected_to(get(conn, old_url), :moved_permanently) =~ stop_path(SiteWeb.Endpoint, :show, :subway)
    end

    test "Commuter stop redirected to commuter rail stops page", %{conn: conn} do
      old_url = "schedules_and_maps/rail/lines/stations/"
      assert redirected_to(get(conn, old_url), :moved_permanently) =~ stop_path(SiteWeb.Endpoint, :show, :commuter_rail)
    end

    test "Ferry stop redirected to ferry stops page", %{conn: conn} do
      old_url = "schedules_and_maps/boats/lines/stations/"
      assert redirected_to(get(conn, old_url), :moved_permanently) =~ stop_path(SiteWeb.Endpoint, :show, :ferry)
    end
    ...
    test "redirects to /maps for system_map", %{conn: conn} do
      assert redirected_to(get(conn, "/schedules_and_maps/system_map"), :moved_permanently) == "/maps"
    end
