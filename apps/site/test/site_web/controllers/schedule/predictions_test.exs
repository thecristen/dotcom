defmodule SiteWeb.ScheduleController.PredictionsTest do
  ...
  describe "call/2" do
    ...
    test "ignores predictions which have the origin as their destination", %{conn: conn} do
      prediction = %Predictions.Prediction{time: ~N[2017-01-01T00:00:00], stop: %Stops.Stop{id: "origin"},
                                           trip: 1234, departing?: false}
      conn = conn
    ...
    test "does not ignore predictions which have a trip id but not status", %{conn: conn} do
      prediction = %Predictions.Prediction{time: ~N[2017-01-01T00:00:00], stop: %Stops.Stop{id: "destination"},
                                           status: nil, trip: 1234, departing?: false}
      conn = conn
      |> assign(:origin, %Stops.Stop{id: "origin"})
      |> assign(:destination, nil)
      |> assign(:route, %{id: "4"})
    ...
    test "does not ignore predictions which have a status but not a trip id", %{conn: conn} do
      prediction = %Predictions.Prediction{time: ~N[2017-01-01T00:00:00], stop: %Stops.Stop{id: "destination"},
                                           status: "On Time", trip: nil, departing?: false}
      conn = conn
      |> assign(:origin, %Stops.Stop{id: "origin"})
      |> assign(:destination, nil)
      |> assign(:route, %{id: "4"})
    ...
    test "ignores predictions which do not have a trip id or a status", %{conn: conn} do
      prediction = %Predictions.Prediction{time: ~N[2017-01-01T00:00:00], stop: %Stops.Stop{id: "destination"},
                                           status: nil, trip: nil, departing?: false}
      conn = conn
      |> assign(:origin, %Stops.Stop{id: "origin"})
      |> assign(:destination, nil)
      |> assign(:route, %{id: "4"})
