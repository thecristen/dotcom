defmodule SiteWeb.ScheduleV2Controller.TripInfoTest do
  ...
  describe "show_trips?/4" do
    test "it is false when looking at a future date for subway" do
      ...
      assert SiteWeb.ScheduleV2Controller.TripInfo.show_trips?(next_day, @time, 1, "1") == false
    end

    test "is true when looking at the subway today" do
      assert SiteWeb.ScheduleV2Controller.TripInfo.show_trips?(@time, @time, 1, "1") == true
    end

    test "has the same behavior for light rail as for subway" do
      next_day = Timex.shift(@time, days: 1)
      assert SiteWeb.ScheduleV2Controller.TripInfo.show_trips?(@time, @time, 0, "1") == true
      assert SiteWeb.ScheduleV2Controller.TripInfo.show_trips?(next_day, @time, 0, "1") == false
    end
