defmodule JourneyTest do
  ...
  describe "Journey.display_status/1" do
    test "returns the same as Journey.display_status/2 with a nil second argument" do
      assert Journey.display_status(%PredictedSchedule{schedule: nil, prediction: %Prediction{status: "On Time"}}) ==
              Journey.display_status(%PredictedSchedule{schedule: nil, prediction: %Prediction{status: "On Time"}}, nil)
    end
  end

  describe "Journey.display_status/2" do
    test "uses the departure status if it exists" do
      result = Journey.display_status(%PredictedSchedule{schedule: nil, prediction: %Prediction{status: "On Time"}}, nil)
    ...
    test "includes track number if present" do
      ...
      result = Journey.display_status(%PredictedSchedule{schedule: nil, prediction: prediction}, nil)
    ...
    test "returns a readable message if there's a difference between the scheduled and predicted times" do
      ...
      result =
        Journey.display_status(
          %PredictedSchedule{schedule: %Schedule{time: now}, prediction: %Prediction{time: later}},
    ...
    test "takes the max of the departure and arrival time delays" do
      ...
      result =
        Journey.display_status(
          %PredictedSchedule{schedule: %Schedule{time: departure}, prediction: %Prediction{time: later_departure}},
          %PredictedSchedule{schedule: %Schedule{time: arrival}, prediction: %Prediction{time: later_arrival}}
      )
  ...
  describe "Journey.before/2" do
    test "compares by departures" do
      ...
      journey3 = %Journey{departure: %PredictedSchedule{schedule: %Schedule{time: ~N[2017-03-01T09:00:00]}}}
    ...
    test "compares by arrival when departure is nil" do
      journey1 =
        %Journey{departure: nil,
                  arrival: %PredictedSchedule{schedule: %Schedule{time: ~N[2017-03-01T10:00:00]}}}
      journey2 =
        %Journey{departure: %PredictedSchedule{schedule: %Schedule{time: ~N[2017-03-01T08:00:00]}},
    ...
    test "compares when departure and arrival is nil" do
      dep_time_nil =
        %Journey{departure: nil,
                  arrival: %PredictedSchedule{schedule: %Schedule{time: ~N[2017-03-01T10:00:00]}}}
      arr_time_nil =
        %Journey{departure: %PredictedSchedule{schedule: %Schedule{time: ~N[2017-03-01T10:00:00]}},
                  arrival: nil}
    ...
    test "compares by status if there are no times" do
      one_away = %Journey{departure: %PredictedSchedule{prediction: %Prediction{status: "1 stop away"}}}
      two_away = %Journey{departure: %PredictedSchedule{prediction: %Prediction{status: "2 stops away"}}}
      boarding = %Journey{departure: %PredictedSchedule{prediction: %Prediction{status: "Boarding"}}}
      approaching = %Journey{departure: %PredictedSchedule{prediction: %Prediction{status: "Approaching"}}}
  ...
  describe "Journey.departure_schedule_before?/2" do
    test "compares with time" do
      journey = %Journey{departure: %PredictedSchedule{schedule: %Schedule{time: ~N[2017-03-01T07:30:00]}}}
