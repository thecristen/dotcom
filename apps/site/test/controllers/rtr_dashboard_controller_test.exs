defmodule RtrDashboardControllerTest do
  ...
  describe "RtrDashboardData" do
    test "makes a request to the right api" do
      ...
      query_string = "?api_key=rMKswlBRmEGhsziJHxx6Pg&format=json&from_service_date=#{current_date}&route=Mattapan&to_service_date=#{current_date}"
    ...
    test "requests given line" do
      current_date = Timex.shift(Util.service_date, days: -1)
      base_url = Application.get_env(:site, :rtr_accuracy_api_url)
      query_string = "?api_key=rMKswlBRmEGhsziJHxx6Pg&format=json&from_service_date=#{current_date}&route=Red&to_service_date=#{current_date}"
      with_mock HTTPoison, [get: fn(_params) -> {:ok, []} end] do
        Site.RtrDashboardData.get(%{"route" => "Red"})
        assert called HTTPoison.get(base_url <> query_string)
      end
    end
  end
