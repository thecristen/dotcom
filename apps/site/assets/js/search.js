...
function setupFilterToggleOnMobile ($, breakpoints) {
  ...
  $("[data-heading='search-filter']").addClass("closed").click((ev) => {
    ...
    const $listContainer = $header.siblings("[data-container='search-filter']").eq(0);
    if ($listContainer.hasClass("hidden-sm-down")) {
      $listContainer.hide().removeClass("hidden-sm-down").slideDown("fast");
