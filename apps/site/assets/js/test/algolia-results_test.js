...
describe("AlgoliaResults", () => {
  ...
  describe("onClickResultCallback", () => {
    it("updates the href with parameters", () => {
      ...
      expect(window.location.href).to.equal("/link?from=global-search");
    });
  });
  ...
  describe("_showLocation", () => {
    it("adds query parameters for analytics", () => {
      ...
      expect(window.location.href).to.contain("from=global-search");
      expect(window.location.href).to.contain("latitude=42.0");
      expect(window.location.href).to.contain("longitude=-71.0");
