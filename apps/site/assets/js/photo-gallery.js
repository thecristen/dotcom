...
export default function photoGallery ($) {
  $ = $ || window.jQuery;
  document.addEventListener("turbolinks:load", () => handlePageLoad($), {passive: true});
...
const makeGallery = ($el, images) => ({el: $el, images: images, imageOffset: 0, pageOffset: 0,
                                       lastPage: calculateLastPage(images.length)});

const calculateLastPage = (photoCount) => Math.max(0, Math.ceil(photoCount / PAGE_SIZE) - 1);
...
function render (id, focusId) {
  // get main image
  const mainImage = galleries[id].images.filter((_el, offset) => offset == galleries[id].imageOffset).pop();
