defmodule Site.ScheduleV2View do
  ...
  @spec render_trip_info_stops([{{PredictedSchedule.t, boolean}, non_neg_integer}], map, boolean) :: [Phoenix.HTML.Safe.t]
  def render_trip_info_stops(stop_list, assigns, collapsed_stops?) do
    for {{predicted_schedule, is_terminus?}, idx} <- stop_list do
      ...
      bubbles: [{assigns.trip_info.route.name, (if is_terminus?, do: :terminus, else: :stop)}],
                ...
                is_expand_link?: idx == Enum.count(stop_list) - 1 and collapsed_stops?
      })
    end
  end
end
