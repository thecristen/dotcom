defmodule Site.RedirectController do
  ...
  def show(conn, %{"path" => redirect_parts} = params) do
    ...
    turbolinks_body_attr = if from_turbolinks?(conn), do: nil, else: "data-turbolinks=\"false\""
  ...
  @spec from_turbolinks?(Plug.Conn.t) :: boolean
  defp from_turbolinks?(conn) do
    Enum.any?(conn.req_headers, fn {header, _} -> header == "turbolinks-referrer" end)
  end
