defmodule SiteWeb.ScheduleV2Controller.Green do
  ...
  plug :route
  plug SiteWeb.ScheduleV2Controller.DatePicker
  plug :alerts
  ...
  def trip_view(conn, _params) do
    ...
    |> render(SiteWeb.ScheduleV2View, "show.html", [])
  end

  def line(conn, _params) do
    conn
    |> assign(:tab, "line")
    |> call_plug(SiteWeb.ScheduleV2Controller.HoursOfOperation)
    |> call_plug(SiteWeb.ScheduleV2Controller.Holidays)
    |> call_plug(SiteWeb.ScheduleV2Controller.Line)
    |> render(SiteWeb.ScheduleV2View, "show.html", [])
  end
  ...
  def schedules(conn, opts) do
    ...
    |> Task.async_stream(fn conn ->
      call_plug(conn, SiteWeb.ScheduleV2Controller.Schedules, opts).assigns.schedules
    ...
    |> SiteWeb.ScheduleV2Controller.Schedules.assign_frequency_table(schedules)
  end

  def predictions(conn, opts) do
    {predictions, vehicle_predictions} =
    if SiteWeb.ScheduleV2Controller.Predictions.should_fetch_predictions?(conn) do
      predictions_fn = opts[:predictions_fn] || &Predictions.Repo.all/1
      predictions_stream = conn
      |> conn_with_branches
      |> Task.async_stream(fn conn ->
        SiteWeb.ScheduleV2Controller.Predictions.predictions(conn, predictions_fn)
      end, timeout: @task_timeout, on_timeout: :kill_task)
      vehicle_predictions = SiteWeb.ScheduleV2Controller.Predictions.vehicle_predictions(conn, predictions_fn)
  ...
  def vehicle_locations(conn, opts) do
    ...
    |> conn_with_branches
    |> Task.async_stream(fn conn ->
      call_plug(conn, SiteWeb.ScheduleV2Controller.VehicleLocations, opts).assigns.vehicle_locations
    end, timeout: @task_timeout)
