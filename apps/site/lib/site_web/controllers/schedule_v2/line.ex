defmodule SiteWeb.ScheduleV2Controller.Line do
  @behaviour Plug
  ...
  @spec get_route_stops(Routes.Route.id_t, direction_id, Stops.Repo.stop_by_route) :: stops_by_route
  ...
  @spec do_get_route_stops(Routes.Route.id_t, direction_id, Stops.Repo.stop_by_route) :: stops_by_route
  ...
  """
  @spec get_branches([Routes.Shape.t], stops_by_route, Routes.Route.t, direction_id) :: [RouteStops.t]
  ...
  defp dedupe_green_stop_list(route_stops) do
    beginnings =
      ...
      |> Enum.map(fn {id, duplicate_stops} ->
        {id,
          Enum.any?(duplicate_stops, &(&1.is_beginning?))
        }
      end)
      |> Enum.into(Map.new)
  ...
  defp add_wollaston_on_red_line([{_, %Stops.RouteStop{route: %Routes.Route{id: "Red"}}} | _tail] = stop_list, nil) do
