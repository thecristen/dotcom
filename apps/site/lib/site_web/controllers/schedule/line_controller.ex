defmodule SiteWeb.ScheduleController.LineController do
  ...
  def services(route_id, service_date, services_by_route_id_fn \\ &ServicesRepo.by_route_id/1) do
    ...
    |> Enum.map(&tag_current_services/1)
    |> tag_default_service()
  end
  ...
  defp tag_current_services(service) do
    service_date_string = Date.to_iso8601(service.service_date)

    in_range? = service_date_between?(service)
    ...
    valid_for_today? = (in_range? || added_in?) && !removed?

    Map.put(service, :current_service?, valid_for_today?)
  end
  ...
  defp service_date_between?(%{start_date: a, end_date: b, service_date: target}) do
    comparisons = [Date.compare(a, target), Date.compare(target, b)]
    Enum.all?(comparisons, &(&1 !== :gt))
  end
