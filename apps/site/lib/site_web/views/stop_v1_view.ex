defmodule SiteWeb.StopV1View do
  @moduledoc """
  Stop page display functions.
  """
  use SiteWeb, :view
  ...
  alias SiteWeb.{AlertView, Endpoint}
  ...
  def location(%Stop{latitude: nil, address: address}),
    do: URI.encode(address, &URI.char_unreserved?/1)

  def location(%Stop{latitude: lat, longitude: lng}), do: "#{lat},#{lng}"
  ...
  @spec accessibility_info(Stop.t(), [Route.gtfs_route_type()]) :: Safe.t()
  ...
  @spec format_accessibility_prefix(Stop.t(), [Route.gtfs_route_type()]) ::
          Safe.t()
  ...
  @spec format_accessibility_options(Stop.t()) :: Safe.t()
  ...
  @spec format_accessibility_text(Stop.t(), [Route.gtfs_route_type()]) :: iodata
  ...
  @spec bus_route?([Route.gtfs_route_type()]) :: boolean
  defp bus_route?(route_types), do: Enum.any?(route_types, &(&1 == :bus))

  @spec accessibility_contact_link(Stop.t()) :: Safe.t()
  defp accessibility_contact_link(stop) do
    case stop.accessibility do
      ...
      _ ->
        content_tag :p do
          link to: customer_support_path(Endpoint, :index), class: "c-call-to-action" do
  ...
  @spec has_alerts?([Alert.t()], InformedEntity.t()) :: boolean
  @doc "Returns true if the given route has alerts."
  def has_alerts?(alerts, informed_entity) do
    alerts
    |> Enum.reject(&(&1.priority == :low))
    |> Match.match(informed_entity)
    |> Enum.empty?()
    |> Kernel.not()
  end

  @spec station_schedule_empty_msg(atom) :: Safe.t()
  ...
  @spec time_differences([PredictedSchedule.t()], DateTime.t()) :: [Safe.t()]
  ...
  def time_differences(predicted_schedules, date_time) do
    ...
    |> Enum.flat_map(fn departure ->
      case Display.time_difference(departure, date_time) do
        "" -> []
  ...
  @spec info_tab_name([Group.t()]) :: String.t()
  ...
  @spec render_header_modes(Stop.t(), Keyword.t(), integer | nil) :: [Safe.t()]
  ...
  @spec render_subway_route_icon(Route.t(), Stop.t()) :: Safe.t()
  ...
  @spec render_green_line_icons([Route.t()], Stop.t()) :: [Safe.t()]
  ...
  @spec do_render_header_mode(Safe.t(), String.t(), Route.t(), Stop.t()) ::
          Safe.t()
  ...
  defp stop_schedule_anchor_link(<<mode::binary>>, %Stop{} = stop) do
    Endpoint
    |> stop_v1_path(:show, stop, tab: "departures")
    |> URI.parse()
    |> Map.put(:fragment, mode <> "-schedule")
    |> URI.to_string()
  end

  @spec render_header_mode_name(Route.t(), String.t()) :: [Safe.t()]
  ...
  defp header_anchor_path(%Stop{} = stop, <<anchor::binary>>) do
    Endpoint
    |> stop_v1_path(:show, stop, tab: "info")
    |> URI.parse()
