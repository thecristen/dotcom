defmodule SiteWeb.Views.Helpers.StopHelpers do
  ...
  def special_disabled_stops(:origin), do: [{"Foxboro", "Trains depart 30 minutes after conclusion of events"}]
  ...
  @spec excluded_stops(:origin | :destination, Routes.Route.t, [Stops.Stop.t], 0 | 1) :: [{String.t, String.t}]
  def excluded_stops(:origin, route, excluded_stops, direction_id) do
    Enum.map(excluded_stops, &{&1, "last #{route |> Routes.Route.direction_name(direction_id) |> String.downcase} stop"})
  end
  ...
  """
  @spec capitalized_disabled_text([{Stops.Stop.id_t, String.t}], Stops.Stop.t) :: String.t | nil
