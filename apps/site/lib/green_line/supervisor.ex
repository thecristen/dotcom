defmodule Site.GreenLine.Supervisor do
  ...
  It also supervises a CacheWarmer which, at application start, spins up an
  agent for every day from today until `Schedules.Repo.end_of_rating`. The
  warmer also schedules itself to run again at 7am ET the next morning.
  ...
  def init(_) do
    children = [
      ...
      worker(Site.GreenLine.CacheWarmer, []),
    ]
