defmodule GoogleMapsTest do
  ...
  describe "unsigned_url/2" do
    test "appends the client ID to the URL" do
      assert GoogleMaps.unsigned_url("/maps/api/staticmap/?arg", client_id: "test", signing_key: @signing_key) ==
  ...
  describe "direction_map_url/2" do
    test "returns a correctly formatted url" do
      url = GoogleMaps.direction_map_url({@location_1.lat, @location_1.lng}, {@location_2.lat, @location_2.lng})
      assert url == "https://maps.google.com" <> Path.join([
                                                    "/", "maps", "dir",
