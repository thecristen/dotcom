...
1. Running the tests generates a new folder in
    `apps/site/test/backstop_data/bitmaps_test/` with images for each test that
...
1. For each image that failed as expected, delete the corresponding image in
   `apps/site/test/backstop_data/bitmaps_reference` and replace it with the new
...
1. The new mappings will be generated in `apps/site/test/mappings` and `apps/site/test/__files`
